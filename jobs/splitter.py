    ...
    def execute(self, context):
        ...
        for input_file in input_files:
            output_pattern = common.append_suffix_to_filename(
                input_file, Splitter.SUFFIX)
            sox.split(
                input_file,
                output_pattern,
                fragment_duration_in_sec=self.fragment_duration_in_sec)

            # 1. sox starts indexing with 1.
            # 1. if total duration is 31s and fragment duration is 10s then sox
            # will create 4 fragments.
            fragment_count = int(
                math.ceil(
                    self.duration_in_sec /
                    self.fragment_duration_in_sec))
            for fragment_index in range(1, fragment_count + 1):
                output_files.append(
                    output_pattern.replace(
                        '@n', str(fragment_index)))
